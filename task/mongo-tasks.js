    ...
    const result = await db.collection('order-details').aggregate([
        {
            $group: {
                ...
                Total_Discount: {$sum: {$multiply: ["$Discount", "$Quantity"]}}
            }
        },
        {
            $project: {
                ...
                "Total Order Discount, %": {$round: [{$multiply: [{$divide: ["$Total_Discount", "$Total_Price"]}, 100]}, 3]}
            }
        },
        {
            $sort: {
    ...
    const result = await db.collection('orders').aggregate([
        ...
        },
        {
            $project: {
                ...
                "TotalOrdersAmount, $": {$round: ["$TotalCount", 2]}
            }
        },
    ...
    const result = await db.collection('orders').aggregate([
        ...
        },
        {
            $project: {
                _id: 0,
                "EmployeeID": "$_id",
                "Employee Full Name": {$arrayElemAt: ["$employeeName", 0]},
                "Amount, $": {$round: ["$TotalCount", 2]}
            }
        },
        {$sort: {"Amount, $": -1}},
        {$limit: 1}
    ...
    const result = await db.collection('order-details').aggregate([
        ...
        },
        {
            $project: {
                _id: 0,
                OrderID: "$_id",
                "Maximum Purchase Amount, $": "$OrderCost"
            }
        },
        { 
            $sort: { 
